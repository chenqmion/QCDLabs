...
dr_readout.wave.add(name='x2', t_data=[0, 0.25, 0.5, 0.75, 1.0], s_data=[0, 1, 1.5, 1, 0], idx=-1, interp_order=3)
...
iq_mat2 = []
volt_list = np.arange(0, 2.5+1e-6, 0.25)
# volt_list = [0, 0.1]
for volt_flux in tqdm(volt_list):
    sim.slow_set(volt_flux, dv=0.01)
    time.sleep(1)
    ...
    iq_mat2.append(iq_mat)

    plt.plot(iq_mat.frequency / 1e9, 20 * np.log10(np.abs(iq_mat.sel(rox=1, reps=1))), '.-')

iq_mat2 = xr.concat(iq_mat2, dim=xr.DataArray(volt_list, name='voltage', dims='voltage'))
...
# plt.figure()
# plt.plot(iq_mat2.frequency/1e9, 20*np.log10(np.abs(iq_mat2.sel(rox=1, reps=1, voltage=0))), '.-')
# plt.xlim(6.75, 6.785)
# plt.show()

plt.figure()
plt.pcolormesh(iq_mat2.frequency/1e9, iq_mat2.voltage, 20*np.log10(np.abs(iq_mat2.sel(rox=1, reps=1))))
# plt.xlim(6.75, 6.785)
plt.show()